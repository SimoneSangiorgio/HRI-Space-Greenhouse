...
class MainRobotController:
    ...
    def execute_command(self, parsed_command):
        ...
        # --- MODIFICA 2: Espansione della logica di pianificazione ---
        # Se il comando è una sequenza, controlla ogni task al suo interno
    ...
    def _perform_physical_greeting(self):
        ...
        if not home_success or self.play_motion_client.get_state() != actionlib.GoalStatus.SUCCEEDED:
            rospy.logerr("CRITICAL: Failed to return arm to 'home' position. The arm may be in an unexpected state.")
        else:
    ...
    def handle_execute_task_sequence(self, command):
        ...
        sequence_name = command.get("sequence_name")
        if sequence_name:
            self.say_and_print(f"Acknowledged. Starting the '{sequence_name}' sequence.", is_robot_response=True)
            task_list = self.kg.get_predefined_sequence(sequence_name)
            if not task_list:
                self.say_and_print(f"Error: I don't know the sequence named '{sequence_name}'.", is_robot_response=True)
                return False
        if not task_list:
    ...
    def handle_navigate_to_location(self, command):
        ...
        if not success: self.kg.log_interaction(self.current_user_id, command.get("intent"), success=False)
        return success
    ...
    def handle_image_analysis(self, command):
        if command.get('target'):
            rospy.logwarn("handle_image_analysis called directly with a target, but should have been planned.")
    ...
    def handle_recommendation(self, command):
        ...
        recommendation_json_str = self.llm.get_recommendation(
        ...
        try:
            ...
            if task_to_execute and task_to_execute.get("intent"):
                ...
                if target_name:
                    prompt += f" on target '{target_name}'"
